...
const WORDMARK_PATHS = `
  ...
  <path d="M176.7,28a4.188,4.188,0,1,1-4.188-4.188A4.188,4.188,0,0,1,176.7,28" transform="translate(-82.719 -11.703)"/>
  <path d="M687.391,28.941a4.188,4.188,0,1,1-4.188-4.188,4.188,4.188,0,0,1,4.188,4.188" transform="translate(-333.693 -12.164)"/>
  <path d="M211.539,31.738H198.312v34.32h-7.983V31.738h.077v-7.05h21.132Z" transform="translate(-93.535 -12.133)"/>
`;

/**
 * Devuelve el wordmark TrueOmni como string SVG válido y autónomo, con el color
 * `fill` ya resuelto a un literal (por defecto blanco, como en el footer del
 * Billboard idle). Lo usa el export standalone (#4) para materializar
 * `Powered-by-TrueOmni.svg`. DRY: comparte `WORDMARK_PATHS` con el componente.
 */
export function trueOmniWordmarkSvg(color = '#ffffff'): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 353.697 65.369" fill="${color}">${WORDMARK_PATHS}</svg>`;
}
...
function TrueOmniLogoBase({
  ...
  return (
    ...
    >
      <title>{title}</title>
      {/*
        Paths verbatim (wordmark + icono + letra T) en `WORDMARK_PATHS` — única
        fuente de verdad compartida con `trueOmniWordmarkSvg()`. Se inyectan con
        dangerouslySetInnerHTML para no duplicar los ~18 paths.
      */}
      <g dangerouslySetInnerHTML={{ __html: WORDMARK_PATHS }} />
    </svg>
  );
}
