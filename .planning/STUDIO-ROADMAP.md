...
- [x] **S5.1 — Bridge live preview ads** ✅ cerrada 2026-04-29 — `pushAds` en `usePreviewBridge` con debounce 150ms + sender `studio:ads-update`. Re-emit en `studio:ready` para cubrir race del iframe re-mount. Listener en `StudioBridge` traduce a `kiosk:ads-override` event. `useAds` consume el override con state local que sustituye el catálogo del config — cambios visibles en preview <300ms sin Save.
