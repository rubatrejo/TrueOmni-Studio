...
import type { VideoWallSlideSchedule } from '@/lib/video-walls/schema';

/**
 * `<SchedulePopover>` (Video Walls) — clone del SchedulePopover del DD.
 *
 * Soporta `always` y `hours` (con startTime + endTime). `date-range` y
 * `daysOfWeek` quedan fuera de scope (mismo criterio que DD/DSS4).
 */
export interface SchedulePopoverProps {
  schedule: VideoWallSlideSchedule;
  onApply: (schedule: VideoWallSlideSchedule) => void;
  onClose: () => void;
...
export function SchedulePopover({ schedule, onApply, onClose, anchorRect }: SchedulePopoverProps) {
  ...
  function handleApply() {
    if (kind === 'always') {
      ...
      onApply({ kind: 'hours', startTime, endTime, hideOutsideSchedule: true });
    }
    onClose();
  }
