...
/**
 * Editor del módulo Listings — 3 sub-tabs (Restaurants / Things to Do / Stay).
 * Cada sub-tab compone hero + 2 taxonomies (subcategories, features) +
 * toolbar con search/filter + lista de items con form completo.
 */
  ...
  return (
    <div className="space-y-4">
      <header className="space-y-1">
        <h2 className="font-display text-[20px] font-semibold text-zinc-100">Listings</h2>
      ...
      {/* Sub-tabs */}
      <div className="flex gap-1 rounded-md border border-zinc-800 bg-zinc-900/40 p-1">
        {TABS.map(({ key, label }) => {
...
function ListingsCatalogEditor({
  ...
  const handleReorder = (next: ListingItem[]) => {
    // El reorder afecta sólo a los visibles — preserva el orden relativo del resto.
      ...
      <CatalogList<ListingItem>
        ...
        renderForm={(item, onChangeItem) => (
          <CatalogItemForm<ListingItem>
            ...
            fields={fields}
            onChange={(patch) => {
              // priceRange viene del select como string — convertir.
              if (patch.priceRange !== undefined) {
                const num = Number(patch.priceRange) as 1 | 2 | 3 | 4;
                onChangeItem({ ...patch, priceRange: num });
              } else {
                onChangeItem(patch);
              }
            }}
