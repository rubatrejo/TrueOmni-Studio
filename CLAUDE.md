...
- ❌ Añadir una dependencia con binarios nativos (sharp, etc.) sin contar su peso contra el **cap de 250 MB de la lambda serverless de Vercel**. Lección 2026-06-12: añadí `sharp` para el placeholder 16:9; el deploy del feature (`82a32ed`) entró en **ERROR** porque la lambda `api/studio/clients` ya carga **~190 MB de assets fs de `clients/*`** (los sirve el catch-all `[...path]/route.ts`) y `sharp` instala libvips para glibc **Y musl** (+32 MB), total 252.45 MB > 250 MB. Costó un segundo ciclo de build (`08356ef`). **Regla:** antes de pushear un cambio que añade peso a `node_modules`, prevé el impacto en lambdas que ya van cargadas (el grafo/STATE recuerda que los assets fs de `clients/*` pesan mucho); el fix de sharp fue `outputFileTracingExcludes` de la variante musl (Vercel corre glibc). **Deuda de fondo:** servir los assets fs de `clients/*` como estáticos o desde Blob en vez de empaquetarlos en cada lambda.
- ❌ Verificar el Studio con clicks manuales uno-por-uno de `agent-browser` cuando el flujo es repetitivo o el daemon se traba (`Resource temporarily unavailable`). Lección 2026-06-12: el QA del placeholder (crear cliente → generar → regenerar → purgar) se peleó con el daemon y consumió ~20 min de round-trips. **Regla:** para flujos E2E repetibles del Studio usa el **spec scriptable** (`agent-browser batch --bail < tests/e2e/<spec>.json`, ver `tests/e2e/studio-create-client.json`); deja los clicks manuales para inspección puntual. Si un click no registra, cae a `agent-browser eval` con el native value setter en vez de reintentar el mismo click.
- ❌ Añadir un campo nuevo a un schema con `.default()` y verificarlo SOLO en el runtime. El runtime aplica defaults (p. ej. `withDefaults` del billboard), pero los EDITORES del Studio suelen acceder al campo directo desde el estado hidratado del KV — y un config de cliente guardado ANTES del campo NO lo trae → `undefined.x` crashea el editor con "Something went wrong". Lección 2026-06-15: añadí `touchHere.radius/textColor/...` y el editor de `hello-harford` (config viejo) crasheó. **Regla:** al añadir campos a un schema, deep-mergea los defaults al construir el estado del editor (como ya se hace con `background` en `BillboardEditor`), y verifica el editor con un config VIEJO, no solo el render del runtime.
- ❌ Pushear una feature que use hooks de `next/navigation` (o tipos sensibles de Next) confiando solo en que `pnpm typecheck` local pasa. El `tsc` local puede tipar más laxo que el `next build` de Vercel: lección 2026-06-18 — `usePathname()` local tipa `string`, pero el build de Vercel lo tipa `string | null` → el deploy entró en ERROR ("'pathname' is possibly null") pese a typecheck + lint limpios; costó un segundo ciclo de build. **Regla:** para features que tocan `next/navigation` (usePathname/useSearchParams/etc.) o tipos sensibles de Next, validar con `pnpm build` local (corre el mismo "Checking validity of types" que Vercel) ANTES de pushear; `pnpm build` requiere aprobación de Rubén — pídela, no te la saltes.
